...
public sealed class MatchmakingEngine(IMatchmakingStrategy strategy, ILogger<MatchmakingEngine> log)
    : BackgroundService
{
    private readonly Channel<PlayerTicket> _incoming = Channel.CreateUnbounded<PlayerTicket>();
    ...
    public ValueTask EnqueueAsync(PlayerTicket t, CancellationToken ct = default) =>
        _incoming.Writer.WriteAsync(t, ct);
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                {
                    if (_playerToMatch.ContainsKey(t.PlayerId))
                        continue;
                    if (_inPool.TryAdd(t.PlayerId, 1))
                ...
                if (!reader.TryRead(out var next))
                {
                    try
                    {
                        next = await reader.ReadAsync(stoppingToken); // blocks until new ticket arrives
                    }
                    ...
                    if (_playerToMatch.ContainsKey(next.PlayerId))
                        continue;
                    if (_inPool.TryAdd(next.PlayerId, 1))
                        strategy.AddTicket(next with { EnqueuedAtUtc = DateTime.UtcNow });
                }
            }
            catch (OperationCanceledException)
            {
